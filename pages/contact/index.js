                ...
                <Grid container spacing={1} alignItems="center" justify="center">
                    <Grid item xs={8} >
                        <Autocomplete
                            ...
                            id="grouped-demo"
                            options={options.sort((a, b) => -b.firstName.localeCompare(a.firstName))}
                                ...
                                (itemsFiltered && itemsFiltered.length) ? itemsFiltered.map((r, i) =>
                                    <ListItem style={{ width: '100%', padding: 10 }} autoFocus={i == 0 ? true : false} key={r.id}  >
                                        <ListItemAvatar style={{ maxHeight: 200, overflow: 'auto', padding: 10, 'marginRight': '20px' }}>
                                            <Avatar>{formatLetter(r.firstName, r.lastName)}</Avatar>
                                        </ListItemAvatar>
                                        <ListItemText style={{ maxHeight: 200, overflow: 'auto', padding: 1 }} primary={(!r.firstName && !r.lastName) ? r.phone : formatText(r.firstName, r.lastName)} secondary={(!r.firstName && !r.lastName) ? '' : r.phone} />
                                        <IconButton onClick={()=>redirectTo(r.key)} edge="end" style={{ maxHeight: 200, overflow: 'auto', padding: 10, 'marginRight': '20px' }} aria-label="delete">
                                            <DeleteIcon />
                                        </IconButton>
                                    </ListItem> 
                                ):<React.Fragment>
